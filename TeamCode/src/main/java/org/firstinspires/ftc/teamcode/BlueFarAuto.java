...
public class BlueFarAuto extends OpMode {
    ...
    public enum AutoState {
        ...
        MOVE_FROM_TOP_BLUE_ARTIFACTS_TO_FINAL_SHOOTING_LINE,
    ...
    public void autoStateUpdate(){
        switch (currentAutoState){
            case MOVE_FROM_START_POS_TO_SHOOTING_LINE:
                driver.moveToPose(DriverDanny.Poses.BFA_FAR_SHOOTING_POSE, true);
            ...
            case MOVE_FROM_SHOOTING_LINE_TO_BLUE_BOTTOM_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()){
                    driver.moveToPose(DriverDanny.Poses.BFA_BOTTOM_ARTIFACTS_POSE, true);
                    setAutoState(AutoState.EAT_BOTTOM_BLUE_ARTIFACTS);
                }
                break;
            case EAT_BOTTOM_BLUE_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()){
                    driver.moveToPose(DriverDanny.Poses.BFA_EAT_BOTTOM_ARTIFACTS_POSE, true);
            ...
            case MOVE_FROM_SHOOTING_LINE_TO_BLUE_MIDDLE_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()){
                    driver.moveToPose(DriverDanny.Poses.BFA_MIDDLE_ARTIFACTS_POSE, true);
                    setAutoState(AutoState.EAT_MIDDLE_BLUE_ARTIFACTS);
                }
                break;
            case EAT_MIDDLE_BLUE_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()) {
                    driver.moveToPose(DriverDanny.Poses.BFA_EAT_MIDDLE_ARTIFACTS_POSE, true);
            ...
            case MOVE_FROM_MIDDLE_BLUE_ARTIFACTS_TO_SHOOTING_LINE:
                if (!driver.isBusy() && !shooter.isShooting()) {
                    driver.moveToPose(DriverDanny.Poses.BFA_NEAR_SHOOTING_POSE, true);
            ...
            case MOVE_FROM_SHOOTING_LINE_TO_BLUE_TOP_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()){
                    driver.moveToPose(DriverDanny.Poses.BFA_TOP_ARTIFACTS_POSE, true);
                    setAutoState(AutoState.EAT_TOP_BLUE_ARTIFACTS);
                }
                break;
            case EAT_TOP_BLUE_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()){
                    driver.moveToPose(DriverDanny.Poses.BFA_EAT_TOP_ARTIFACTS_POSE, true);
                    setAutoState(AutoState.MOVE_FROM_TOP_BLUE_ARTIFACTS_TO_FINAL_SHOOTING_LINE);
                }
                break;
            case MOVE_FROM_TOP_BLUE_ARTIFACTS_TO_FINAL_SHOOTING_LINE:
                if (!driver.isBusy() && !shooter.isShooting()) {
                    driver.moveToPose(DriverDanny.Poses.BFA_FINAL_SHOOTING_POSE, true);
    ...
    public void init() { // this runs once when you hit "init" on the driver hub
        ...
        driver = new DriverDanny(hardwareMap,
                ...
                DriverDanny.Poses.BFA_START_POSE);
